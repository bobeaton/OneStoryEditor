...
REM BE: it's already been built heat.exe dir %APPBUILDDIR% -cg HarvestedAppFiles -gg -scom -sreg -sfrag -srd -sw5150 -sw5151 -dr APPFOLDER -var var.APPBUILDDIR -t KeyPathFix.xsl -out AppHarvest.wxs
REM BE: it's already been built heat.exe dir %APPDATADIR% -cg HarvestedDataFiles -gg -scom -sreg -sfrag -srd -sw5150 -sw5151 -dr HARVESTDATAFOLDER -var var.APPDATADIR -t KeyPathFix.xsl -out DataHarvest.wxs
...
REM BE: it's already been built light.exe Bundle.wixobj -ext WixUIExtension -ext WixBalExtension -ext WixUtilExtension -ext WixNetFxExtension -ext WixDependencyExtension %SuppressICE% -out %SafeAppName%_%Version%_Online.exe

@REM build the OFFLINE EXE bundle.
REM BE: it's already been built candle.exe -arch %Arch% -dApplicationName=%AppName% -dSafeApplicationName=%SafeAppName% -dYear=%CopyrightYear% -dManufacturer=%Manufacturer% -dVersionNumber=%Version% -dUpgradeCode=%UPGRADECODEGUID% -dTruncatedVersion=%TRUNCATEDVERSION% -ext WixUtilExtension -ext WixBalExtension -ext WixUIExtension -ext WixNetFxExtension -ext WixDependencyExtension OfflineBundle.wxs

REM BE: it's already been built light.exe OfflineBundle.wixobj -ext WixUIExtension -ext WixBalExtension -ext WixUtilExtension -ext WixNetFxExtension -ext WixDependencyExtension %SuppressICE% -out %SafeAppName%_%Version%_Offline.exe

@REM Sign the standard installer.
REM BE: it's built by a separate bat file in its own project insignia -ib %SafeAppName%_%Version%_Online.exe -o engine.exe
REM BE: it's built by a separate bat file in its own project call signingProxy engine.exe
REM BE: it's built by a separate bat file in its own project insignia -ab engine.exe %SafeAppName%_%Version%_Online.exe -o %SafeAppName%_%Version%_Online.exe
REM BE: it's built by a separate bat file in its own project call signingProxy %SafeAppName%_%Version%_Online.exe

@REM Sign the offline installer.
REM BE: it's built by a separate bat file in its own project insignia -ib %SafeAppName%_%Version%_Offline.exe -o engine.exe
REM BE: it's built by a separate bat file in its own project call signingProxy engine.exe
REM BE: it's built by a separate bat file in its own project insignia -ab engine.exe %SafeAppName%_%Version%_Offline.exe -o %SafeAppName%_%Version%_Offline.exe
REM BE: it's built by a separate bat file in its own project call signingProxy %SafeAppName%_%Version%_Offline.exe
