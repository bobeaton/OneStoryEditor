...
{
    /// <summary>
    /// I've added this class to provide a way to hook MessageBox.Show calls so that
    /// the OK/Yes, etc buttons can be localized. it should work as is without any
    /// changes to the client application (except that the user must localize the 
    /// relevant strings; see "Localizer.Str" below).
    /// However, I still also added a new Form to do this: LocalizableMessageBox, 
    /// because with this approach, it doesn't adjust the size of the dialog box based
    /// on the size of the message string (and at least with 11 pt Devanagari font, it
    /// clips it too short). So I'm not 100% certain I can recommend this approach.
    /// This approach should work seemlessly so that you don't have to change the client
    /// app, but can localize the MessageBox buttons. With the LocalizableMessageBox 
    /// Form, you have to change all occurrences of "MessageBox.Show" with 
    /// "LocalizableMessageBox.Show" in the client app
    /// </summary>
